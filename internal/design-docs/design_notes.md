...
- The step before the first step might be to process type declarations, to determine
  if they are valid and to write down any properties realizable from their local declaration.
...
- Then we should do a simple crawl of the AST to check scope rules, and maybe build
  ...
  list of locals, so this can either be done inline or as a separate step after.
  I am fond of the separate step, but this means that child scopes need to be visible
...
- Type checking + type unification.
  - Type checking refers to A: determining the types of all entities in a program
  ...
  - Type unification (maybe my term?) will refer to the process of adding conversions into
    ...
    - I am saying unification specifically to invoke Prolog memories of information
      ...
      thing.
    - Perhaps early on, it could be the user's onus to explicitly cast absolutely
      ...
      int promotion).
  - Note: These operations might be inseparable, or at least natural to do together.
...
might reasonably disappear or fuse later), and move existing functionality there while
