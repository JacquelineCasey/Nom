...
//!
//! Organizes compilation, ensuring items are processed in the right order. Provides a type `CompilationEnvironment`
//! that tracks information learned from the program.
//!
//! See README.md for documentation for the project as a whole, as well as for the Nom programming language.
...
///
/// It should be understood that this represents all of the *currently known* information, so at different stages of
/// compilation, different fields should be complete and accurate.
struct CompilationEnvironment {
    ...
    /// Associates function names to information about the function (return type, arguments, etc.).
    functions: HashMap<String, analysis::Function>,

    /// Associates a type to all the information associated with it.
    types: TypeStore,

    /// Associates expressions (by id) to their type. This is filled out during the `type_check` goal.
...
impl CompilationEnvironment {
    ...
    ///
    /// Before calling this, ensure that at least one compilation goal is placed in the queue. Note that finishing one
    /// goal may trigger the addition of follow up goals.
    ...
    ///
    /// Locates data associated with the file, tokenizes and parses it, and generates data about the declarations in the
    /// file. All declarations are parsed and stored, but definitions may or may not be created depending on if they are
    /// needed.
    ///
    ...
    /// Processes a scope check goal, ensuring basic rules are followed and named items exist.
    ///
    /// Confirms that all variables in the function obey scope rules, and const rules. Ensures that uses of external
    /// objects (functions, variables) can be resolved, and adds goals to define them if needed. Builds a list of local
    /// variables.
    ///
    ...
    /// Processes a type check goal, ensuring type rules are followed and determining the type of all expressions.
    ///
    /// After this goal completes, the function is ready to be passed to the code generator.
...
///
/// Each goal has the restriction that, when processed, it can be executed to completion. It can add additional
/// `CompilationGoals`, but they will not run right away. A goal can be enqueued several times, but will be ignored
/// after the first copy is processed.
///
/// The queue should be thought of as a list of uncompleted goals. Inserted completed goals will have no visible effect,
/// since those goals will never be popped later.
struct CompilationQueue {
    /// The list of goals in the order they should be executed. Goals might be placed either at the front or the back
    /// depending on when it should be executed.
    queue: VecDeque<CompilationGoal>,

    /// A set of completed goals, for use to ensure that a given goal is not completed twice.
...
impl CompilationQueue {
    ...
    ///
    /// If it has already been completed by the time it is processed, it will be ignored (i.e. it will not ever be
    /// popped).
    ...
    /// Returns true if and only if there exists a not yet completed goal in the queue.
    ///
...
///
/// We divide these steps so that they can be completed independently as needed for different files, or even items
/// within files. For instance, we may only need to compile some functions from a file, if the others are not used, so
/// we will only scope check those functions.  
///
/// See the functions that implement each of these goals above.
#[derive(PartialEq, Eq, Hash, Debug)]
enum CompilationGoal {
    /// Imports and performs initial processing of a file (or a string). At time of writing, this defines all declared
    /// items, but in the future it might be more selective about only defining needed items. Completion enqueues a
    /// scope check goal for every function defined.
    ImportFile(FileOrString),

    /// Ensures that variables in the function are declared, and other functions exist, and so on. Checks the rules of
    /// the language that can be checked without type information. Should enqueue goals to set other dependencies first,
    /// and then one goal to type check this function.
    ScopeCheck(String),

    /// Type checks a function, and determines the type of every expression within the function. After this runs, the
    /// function is ready to be passed to the code generator.
    TypeCheck(String),
}
...
///
/// Works by placing a single goal to import the input into the compilation queue. Then completes all goals (goals can
/// spawn more goals) until none remain.
...
/// Given a path to a file, tokenizes, parses, analyzes, and generates code for that file.
///
...
/// Given a string input, tokenizes, parses, analyzes, and generates code for that input.
///
/// Returns a list of instructions.
///
/// If an error occurs, it will print to the screen, and the function will panic.
